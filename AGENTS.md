...
before touching any struct. This file covers only the operating contract.
...
Confirm with `../isaac_ros_common/scripts/dexec.sh -- ros2 pkg prefix dji_serial_bridge`. This is C++, so
`--symlink-install` doesn't help, and a source change always needs a rebuild.

## Scope

- Stays a pure UART/DJI-protocol translator: no application logic, and
  nothing but `sentry_pkg`'s `mcb_relay` may publish or subscribe on its
  topics. Anything that wants to reach the MCB goes through that relay;
  adding a direct publisher here is the wrong fix.
